...
public class StatsSystem : MonoBehaviour
{
    ...
    // Declared float dancing stats
    private float rhythmFloat;
    private float luckFloat;
    ...
    public void CalculateDancingStats()
    {
        ...
        luckFloat = (intelligence * intelligenceMultiplier);
        // rhythm should be based off our agility and be converted at a rate of 1 : 0.5f.
        rhythmFloat = (agility * agilityMultiplier);
    }
    ...
    public void ChangeHealth(float amount)
    {
        ...
        // currently we are just automatically removing our player...but we probably only want to do that if
        // there is a character and their health is less than 0.
        if (character != null)
        {
    ...
    public void DistributePhysicalStatsOnLevelUp(int PointsPool)
    {
        ...
        float upgradeStrength = (strength + PointsPool);
        float upgradeAgility = (agility + PointsPool);
        float upgradeIntelligence = (intelligence + PointsPool);
        // then let's recalculate our dancing stats again to process and update the new values.

        int newStyle = (int)(upgradeStrength * style);
        int newRhythm = (int)(upgradeAgility * rhythmFloat);
        int newLuck = (int)(upgradeIntelligence * luckFloat);
    }
    ...
    {
        Debug.Log(string.Format("The new physical stats are str {0}, agil {1}, int {2}, the dancing stats " +
            "are style {3}, luck {4}, rhythm {4}",strength,agility,intelligence,style,luck,rhythm)); 
    }
    #endregion
}
